...
def generate_resume_pdf(author, resume_data):
    ...
    # Extract contact info from resume_data
    contact = resume_data.get('contact', {})
    if isinstance(contact, dict):
        email = contact.get('email', 'abc@xyz.com')
        phone = contact.get('phone', '00-0000000000')
        address = resume_data.get('address', contact.get('location', 'XXX'))
    else:
